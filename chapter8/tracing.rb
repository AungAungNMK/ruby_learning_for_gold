...
Kernel.eval, Object.instance_eval amd Module.class_eval all accept the filename and line number as their 
    final two arguments.
    *When an exception is raised and not handled , error message printed out to the console contain filename and line nummber information based on _LI
    and _FILE_.
    *Every exception Object has a backtrace associated with it that show exactly where is was raised
    when the method raised the exception ,where the method was invoked ,and so on.
    ...
    *Kernel.caller method returns the  current state of the call stack in the same form as Exceprion.backtrace
